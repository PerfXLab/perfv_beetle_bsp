/*
 * Copyright (C) 2019 GreenWaves Technologies
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include "pmsis.h"

#include "bsp/bsp.h"
#include "pmsis/drivers/gpio.h"
#include "pmsis/drivers/pad.h"
#include "bsp/gapuino.h"

#if defined(CONFIG_HIMAX)
#include "bsp/camera/himax.h"
#elif defined(CONFIG_OV7725)
#include "bsp/camera/ov7725.h"
#endif

#if defined(CONFIG_ILI9341)
#include "bsp/display/ili9341.h"
#elif defined(CONFIG_ST7735S)
#include "bsp/display/st7735s.h"
#elif defined(CONFIG_YT280)
#include "bsp/display/yt280L030.h"
#elif defined(CONFIG_Z350)
#include "bsp/display/z350it008.h"
#endif

#if defined(CONFIG_HYPERFLASH)
#include "bsp/flash/hyperflash.h"
#endif

#if defined(CONFIG_HYPERRAM)
#include "bsp/ram/hyperram.h"
#endif

#include "bsp/ram/spiram.h"
#include "bsp/flash/spiflash.h"

#if defined(CONFIG_NINA_W10)
#include "bsp/transport/nina_w10.h"
#elif defined(CONFIG_HC08)
#include "bsp/ble/hc08.h"
#endif

#if defined(CONFIG_HC22)
#include "bsp/wifi/hc22.h"
#endif

static int __bsp_init_pads_done = 0;

static void __bsp_init_pads()
{
  if (!__bsp_init_pads_done)
  {
    __bsp_init_pads_done = 1;
    //uint32_t pads_value[] = {0x00055500, 0x0f000000, 0x003fffff, 0x00000000};
    uint32_t pads_value[] = {0x00055500, 0x0f004000, 0x003fc000, 0x00000000};
    pi_pad_init(pads_value);
  }
}


#if defined(CONFIG_HYPERRAM)
void bsp_hyperram_conf_init(struct pi_hyperram_conf *conf)
{
  conf->ram_start = CONFIG_HYPERRAM_START;
  conf->ram_size = CONFIG_HYPERRAM_SIZE;
  conf->skip_pads_config = 0;
  conf->hyper_itf = CONFIG_HYPERRAM_HYPER_ITF;
  conf->hyper_cs = CONFIG_HYPERRAM_HYPER_CS;
}

int bsp_hyperram_open(struct pi_hyperram_conf *conf)
{
  __bsp_init_pads();
  return 0;
}
#endif

#if defined(CONFIG_HYPERFLASH)
void bsp_hyperflash_conf_init(struct pi_hyperflash_conf *conf)
{
  conf->hyper_itf = CONFIG_HYPERFLASH_HYPER_ITF;
  conf->hyper_cs = CONFIG_HYPERFLASH_HYPER_CS;
}

int bsp_hyperflash_open(struct pi_hyperflash_conf *conf)
{
  __bsp_init_pads();
  return 0;
}
#endif



void bsp_spiflash_conf_init(struct pi_spiflash_conf *conf)
{
  conf->size = CONFIG_SPIFLASH_SIZE;
  // sector size is in number of KB
  conf->sector_size = CONFIG_SPIFLASH_SECTOR_SIZE;
  conf->spi_itf = CONFIG_SPIFLASH_SPI_ITF;
  conf->spi_cs = CONFIG_SPIFLASH_SPI_CS;
  conf->baudrate = 20*1000000;
}

int bsp_spiflash_open(struct pi_spiflash_conf *conf)
{
  __bsp_init_pads();
  return 0;
}



void bsp_spiram_conf_init(struct pi_spiram_conf *conf)
{
  conf->ram_start = CONFIG_SPIRAM_START;
  conf->ram_size = CONFIG_SPIRAM_SIZE;
  conf->skip_pads_config = 0;
  conf->spi_itf = CONFIG_SPIRAM_SPI_ITF;
  conf->spi_cs = CONFIG_SPIRAM_SPI_CS;
}

int bsp_spiram_open(struct pi_spiram_conf *conf)
{
  __bsp_init_pads();
  return 0;
}



#if defined(CONFIG_HIMAX)
void bsp_himax_conf_init(struct pi_himax_conf *conf)
{
  __bsp_init_pads();
  conf->i2c_itf = CONFIG_HIMAX_I2C_ITF;
  conf->cpi_itf = CONFIG_HIMAX_CPI_ITF;
}

int bsp_himax_open(struct pi_himax_conf *conf)
{
  __bsp_init_pads();
  return 0;
}

#elif defined(CONFIG_OV7725)

void bsp_ov7725_conf_init(struct pi_ov7725_conf *conf)
{
  __bsp_init_pads();
  conf->i2c_itf = CONFIG_OV7725_I2C_ITF;
  conf->cpi_itf = CONFIG_OV7725_CPI_ITF;
  conf->reset_gpio = CONFIG_OV7725_GPIO_RST;
  conf->pwdn_gpio = CONFIG_OV7725_GPIO_PWDN;
}

int bsp_ov7725_open(struct pi_ov7725_conf *conf)
{
  __bsp_init_pads();
  if (!conf->skip_pads_config)
  {
    pi_pad_set_function(CONFIG_OV7725_GPIO_RST_PAD, CONFIG_OV7725_GPIO_RST_PAD_FUNC);
    pi_pad_set_function(CONFIG_OV7725_GPIO_PWDN_PAD, CONFIG_OV7725_GPIO_PWDN_PAD_FUNC);
  }
  return 0;
}
#endif




#if defined(CONFIG_ILI9341)
void bsp_ili9341_conf_init(struct pi_ili9341_conf *conf)
{
  conf->gpio = CONFIG_ILI9341_GPIO;
  conf->gpio_rst = CONFIG_ILI9341_GPIO_RST;            //zzj add
  conf->spi_itf = CONFIG_ILI9341_SPI_ITF;
  conf->spi_cs = CONFIG_ILI9341_SPI_CS;

}

int bsp_ili9341_open(struct pi_ili9341_conf *conf)
{
  __bsp_init_pads();

  if (!conf->skip_pads_config)
  {
    pi_pad_set_function(CONFIG_ILI9341_GPIO_PAD, CONFIG_ILI9341_GPIO_PAD_FUNC);
    pi_pad_set_function(CONFIG_ILI9341_GPIO_RST_PAD, CONFIG_ILI9341_GPIO_RST_PAD_FUNC);
  }

  return 0;
}

#elif defined(CONFIG_ST7735S)

void bsp_st7735s_conf_init(struct pi_st7735s_conf *conf)
{
  conf->gpio = CONFIG_ST7735S_GPIO;
  conf->gpio_rst = CONFIG_ST7735S_GPIO_RST;            //zzj add
  conf->spi_itf = CONFIG_ST7735S_SPI_ITF;
  conf->spi_cs = CONFIG_ST7735S_SPI_CS;

}

int bsp_st7735s_open(struct pi_st7735s_conf *conf)
{
  __bsp_init_pads();

  if (!conf->skip_pads_config)
  {
    pi_pad_set_function(CONFIG_ST7735S_GPIO_PAD, CONFIG_ST7735S_GPIO_PAD_FUNC);
    pi_pad_set_function(CONFIG_ST7735S_GPIO_RST_PAD, CONFIG_ST7735S_GPIO_RST_PAD_FUNC);
  }

  return 0;
}

#elif defined(CONFIG_YT280)

void bsp_yt280_conf_init(struct pi_yt280_conf *conf)
{
  conf->gpio = CONFIG_YT280_GPIO;
  conf->gpio_rst = CONFIG_YT280_GPIO_RST;            //zzj add
  conf->spi_itf = CONFIG_YT280_SPI_ITF;
  conf->spi_cs = CONFIG_YT280_SPI_CS;

}

int bsp_yt280_open(struct pi_yt280_conf *conf)
{
  __bsp_init_pads();

  if (!conf->skip_pads_config)
  {
    pi_pad_set_function(CONFIG_YT280_GPIO_PAD, CONFIG_YT280_GPIO_PAD_FUNC);
    pi_pad_set_function(CONFIG_YT280_GPIO_RST_PAD, CONFIG_YT280_GPIO_RST_PAD_FUNC);
  }

  return 0;
}

#elif defined(CONFIG_Z350)

void bsp_z350_conf_init(struct pi_z350_conf *conf)
{
  conf->gpio = CONFIG_Z350_GPIO;
  conf->gpio_rst = CONFIG_Z350_GPIO_RST;            //zzj add
  conf->spi_itf = CONFIG_Z350_SPI_ITF;
  conf->spi_cs = CONFIG_Z350_SPI_CS;

}

int bsp_z350_open(struct pi_z350_conf *conf)
{
  __bsp_init_pads();

  if (!conf->skip_pads_config)
  {
    pi_pad_set_function(CONFIG_Z350_GPIO_PAD, CONFIG_Z350_GPIO_PAD_FUNC);
    pi_pad_set_function(CONFIG_Z350_GPIO_RST_PAD, CONFIG_Z350_GPIO_RST_PAD_FUNC);
  }

  return 0;
}

#endif // defined




void pi_bsp_init_profile(int profile)
{
    __bsp_init_pads();

    if (profile == PI_BSP_PROFILE_DEFAULT)
    {
        /* Special for I2S1, use alternative pad for SDI signal. */
        pi_pad_set_function(PI_PAD_35_B13_I2S1_SCK, PI_PAD_35_B13_I2S1_SDI_FUNC3);
        pi_pad_set_function(PI_PAD_37_B14_I2S1_SDI, PI_PAD_37_B14_HYPER_CK_FUNC3);

#ifndef __ZEPHYR__
        pi_i2s_setup(PI_I2S_SETUP_SINGLE_CLOCK);
#endif
    }
}


#if defined(CONFIG_NINA_W10)
void bsp_nina_w10_conf_init(struct pi_nina_w10_conf *conf)
{
  conf->spi_itf = CONFIG_NINA_W10_SPI_ITF;
  conf->spi_cs = CONFIG_NINA_W10_SPI_CS;
}

int bsp_nina_w10_open(struct pi_nina_w10_conf *conf)
{
  __bsp_init_pads();
  return 0;
}
#endif

#if defined(CONFIG_HC08)
void bsp_hc08_conf_init(struct pi_hc08_conf *conf)
{
    conf->uart_itf = (uint8_t) CONFIG_HC08_UART_ID;
}

int bsp_hc08_open(void)
{
  __bsp_init_pads();
  return 0;
}
#endif


#if defined(CONFIG_HC22)
void bsp_hc22_conf_init(struct pi_hc22_conf *conf)
{
    conf->uart_itf = (uint8_t) CONFIG_HC22_UART_ID;
}

int bsp_hc22_open(void)
{
  __bsp_init_pads();
  return 0;
}
#endif


void pi_bsp_init()
{
  pi_bsp_init_profile(PI_BSP_PROFILE_DEFAULT);
}
