...
typedef struct pi_wifi_api_s
{
    ...
    int32_t (*peer_connect)(struct pi_device *device, const char *addr); /*!< Function to connect WIFI device to a remote peer. */
    int32_t (*peer_disconnect)(struct pi_device *device, const char *addr); /*!< Function to disconnect WIFI device to a remote peer. */
} pi_wifi_api_t;
